...
class TestManagerCommand(sublime_plugin.TextCommand):
	...
	# 错误输出样式
	REGION_ERR_PROP = ['invalid', 'dot', sublime.HIDDEN]
	REGION_ERR_LABEL_PROP = ['comment', 'dot', sublime.HIDDEN]
	# [新增] 标准答案样式 (string 通常是绿色或黄色，适合表示正确答案)
	REGION_EXPECTED_PROP = ['entity.name.function', 'dot', sublime.HIDDEN]
	...
	class Tester(object):
		def __init__(self, process_manager, \
			...
			if type(self.process_manager) != ProcessManager:
				...
				# 默认 ProcessManager 没有 set_calls，但我们添加了 set_err_call
				if hasattr(self.process_manager, 'set_err_call'):
					self.process_manager.set_err_call(self.__on_err)
				# 这里千万不要调用 set_calls
				
			self.prog_err = [''] * len(self.tests)

		def __on_err(self, s):
			if self.running_test is not None and self.running_test < len(self.prog_err):
				self.prog_err[self.running_test] += s

		def __on_stop(self, rtcode, runtime=-1, crash_line=None):
			if self.running_test < len(self.prog_out):
				self.prog_out[self.running_test] = self.prog_out[self.running_test].rstrip()
			if self.running_test < len(self.prog_err):
				self.prog_err[self.running_test] = self.prog_err[self.running_test].rstrip()
		...
		def run_test(self, id):
			...
			while len(self.prog_out) <= id: self.prog_out.append('')
			while len(self.prog_err) <= id: self.prog_err.append('')
		...
		def del_test(self, nth):
			...
			if nth < len(self.prog_err):
				self.prog_err.pop(nth)
		...
		def del_tests(self, to_del):
			...
			for i in range(len(tests)):
				if not i in dont_add:
					...
					new_err.append(err[i] if i < len(err) else '')

			self.prog_out = new_out
			self.prog_err = new_err
	...
	# [关键] 辅助函数：获取标准化的显示内容
	# 统一处理 \r\n 和 \t，确保计算长度与 Sublime 显示一致
	def _get_display_content(self, i):
		tester = self.tester
	...
	# [新增] 通用高亮应用函数
	# 负责在指定的 start_pt 位置重新计算并应用 stderr 和 expected 的高亮
	def _apply_custom_regions(self, i, start_pt):
		v = self.view
		
		# 获取纯净内容（不含末尾的间隔符）
		content = self._get_display_content(i)
		...
		# --- 高亮 Stderr ---
		idx = content.find(sep_err)
		if idx != -1:
			# 1. 标签高亮
			# idx 是 \n\n 的开始，标签文本从 idx + 2 开始
			lbl_start = start_pt + idx + 2
			lbl_end = start_pt + idx + len(sep_err) - 1 # 减1去掉末尾\n
			v.add_regions('test_err_lbl_%d' % i, \
				[Region(lbl_start, lbl_end)], *self.REGION_ERR_LABEL_PROP)

			# 2. 内容高亮
			content_start = start_pt + idx + len(sep_err)
			
			# 寻找下一个块的开始 (Expected)
			idx_next = content.find(sep_exp)
			
			if idx_next != -1:
				content_end = start_pt + idx_next
			else:
				# 如果没有Expected，就到文本结束
				content_end = start_pt + len(content)

			v.add_regions('test_err_%d' % i, \
				[Region(content_start, content_end)], *self.REGION_ERR_PROP)

		# --- 高亮 Expected ---
		idx = content.find(sep_exp)
		if idx != -1:
			# 1. 标签高亮
			lbl_start = start_pt + idx + 2
			lbl_end = start_pt + idx + len(sep_exp) - 1
			v.add_regions('test_exp_lbl_%d' % i, \
				[Region(lbl_start, lbl_end)], *self.REGION_ERR_LABEL_PROP)

			# 2. 内容高亮
			content_start = start_pt + idx + len(sep_exp)
			content_end = start_pt + len(content)
			
			v.add_regions('test_exp_%d' % i, \
				[Region(content_start, content_end)], *self.REGION_EXPECTED_PROP)
	...
	def toggle_fold(self, i):
		...
		# 标准化内容 + 间隔
		text = self._get_display_content(i) + '\n\n'
		...
		if tester.tests[i].fold:
			...
			# 设置基础标记
			inp_len = len(tester.tests[i].test_string.replace('\r\n', '\n').replace('\t', '    '))
			v.add_regions(self.REGION_BEGIN_KEY % i, \
				[Region(tie_pos + 1)], *self.REGION_BEGIN_PROP)
			v.add_regions('test_end_%d' % i, \
				[Region(tie_pos + inp_len + 2, tie_pos + inp_len + 2)], \
					*self.REGION_END_PROP)

			# [关键修正] 重新应用自定义高亮！
			# 文本从 tie_pos + 1 开始插入
			self._apply_custom_regions(i, tie_pos + 1)
			...
			# 清除自定义高亮
			v.erase_regions('test_err_%d' % i)
			v.erase_regions('test_err_lbl_%d' % i)
			v.erase_regions('test_exp_%d' % i)
			v.erase_regions('test_exp_lbl_%d' % i)
	...
	def update_configs(self, update_last=None):
		...
		for i in range(k):
			...
			if running:
				...
				# [关键] 使用标准化长度
				pt += len(self._get_display_content(i))
	...
	def on_stop(self, rtcode, runtime, crash_line=None):
		...
		# 状态判断
		has_err = False
		if test_id < len(tester.prog_err) and tester.prog_err[test_id].strip():
			has_err = True
		
		is_correct = self.tester.tests[test_id].is_correct_answer(self.tester.prog_out[test_id])
	...
	def swap_tests(self, edit, dir=-1):
		...
		for sel in selected:
			if 0 <= sel + dir < len(tester.tests):
				...
				# 交换 prog_err
				tester.prog_err[sel], tester.prog_err[sel + dir] = tester.prog_err[sel + dir], tester.prog_err[sel]
